...
(async () => {
  ...
  for (const [w, h] of [[1920, 1080], [1440, 900]]) {
    ...
    await page.goto(base + "/a/" + assignment, { waitUntil: "networkidle", timeout: 60000 });
    // wait for cockpit labels if present
    try {
      await page.getByLabel("Conversation").first().waitFor({ timeout: 15000 });
    } catch {}
  ...
  await browser.close();
  for (const s of shots) console.log("SHOT " + s + " bytes=" + fs.statSync(s).size);
